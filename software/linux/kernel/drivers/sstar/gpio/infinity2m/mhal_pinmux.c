    ...
    for (i = 0; i < g_u32Padmux_cnt; i++)
    {
        if (u32PadID == m_stPadMuxTbl[i].padID)
        {
            ...
            else
            {
                ...
                {
                    printk(KERN_INFO"[Padmux]reset Pad_%d(reg 0x%x; mask0x%x, val0x%x) to %s(original: %s)\n",
                            u32PadID, m_stPadMuxTbl[i].base+m_stPadMuxTbl[i].offset, m_stPadMuxTbl[i].mask, m_stPadMuxTbl[i].val,
