...
void setupMemory(enum Z80_MEMORY_PROFILE mode)
{
    ...
    else if(mode == USE_VIRTUAL_RAM)
    {
        ...
        for(idx=0x0000; idx < 0x10000; idx++)
        {
            if((idx & 0x00FF) >= IO_ADDR_E0 && (idx & 0x00FF) < IO_ADDR_E7)
            ...
            else
            {
                ...
                switch(ioctlCmd.cmd)
                {
                    ...
                    case IOCTL_CMD_Z80_CPU_FREQ:
                        ...
                        {
                            case 2:
                                Z80Ctrl->cpuGovernorDelay = MZ700_INSTRUCTION_DELAY_7MHZ;
                                break;

                            case 4:
                                Z80Ctrl->cpuGovernorDelay = MZ700_INSTRUCTION_DELAY_14MHZ;
                                break;

                            case 8:
                                Z80Ctrl->cpuGovernorDelay = MZ700_INSTRUCTION_DELAY_28MHZ;
                                break;

                            case 16:
                                Z80Ctrl->cpuGovernorDelay = MZ700_INSTRUCTION_DELAY_56MHZ;
                                break;

                            case 32:
                                Z80Ctrl->cpuGovernorDelay = MZ700_INSTRUCTION_DELAY_112MHZ;
                                break;

                            case 64:
                                Z80Ctrl->cpuGovernorDelay = MZ700_INSTRUCTION_DELAY_224MHZ;
                                break;

                            case 128:
                                Z80Ctrl->cpuGovernorDelay = MZ700_INSTRUCTION_DELAY_448MHZ;
                                break;

                            case 1:
                            default:
                                Z80Ctrl->cpuGovernorDelay = MZ700_INSTRUCTION_DELAY_3_54MHZ;
                                break;
                        }
                        break;
                    ...
                    case IOCTL_CMD_CPLD_CMD:
                        ...
                        SPI_SEND32(ioctlCmd.cpld.cmd);
                        udelay(10);
                        z80io_SPI_Send32(0x00000000, &tmp);
                        pr_info("CPLD RX:%08x\n", tmp);
...
static int __init ModuleInit(void)
{
    ...
    // Copy the host BIOS into the Virtual RAM.
